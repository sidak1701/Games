...
class Brick(PlayComponent):
    colourArray = {1:'lightsteelblue', 2:'royalblue', 3:'blue'}

    def __init__(self, canvas, x, y, hits):
        self.width = 60
        self.height = 20
        self.hits = hits
        colour = Brick.colourArray[hits] 
        item = canvas.create_rectangle(x-self.width/2, y-self.height/2,
                                       x+self.width/2, y+self.height/2,
                                       fill = colour, tag = 'brick') 
    ...
    def hit(self):
        ...
        if self.hits == 0:
            ...
            self.canvas.itemconfig(self.item, fill = Brick.colourArray[self.hits])
...
class Game(tk.Frame):
    ...
    def display_ball(self):
        ...
        self.ball = Ball(self.canvas, x, 490)
